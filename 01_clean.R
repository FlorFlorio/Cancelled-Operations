...
saveRDS(beds_day, "data/processed/day_beds_occupancy_2010Q1_to_2020Q1.rds")
...
beds_overnight <- beds_overnight %>%
  mutate(year = str_sub(year, 1, 4)) %>%
  unite(year_quarter, year, quarter, sep = " ") %>% 
  mutate(year_quarter = as.yearqtr(year_quarter))

saveRDS(beds_overnight, "data/processed/overnight_beds_occupancy_2010Q1_to_2020Q1.rds")
